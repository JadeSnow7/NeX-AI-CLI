...
// map(list, fn) -> List：对 list 中每个元素应用 fn（管道友好：list 在前）
static ValuePtr fn_map(const std::vector<ValuePtr>& args, Evaluator& ev) {
    if (args.size() < 2) return Value::make_error("map: requires (list, fn)");
    if (args[0]->type != Value::Type::List) return Value::make_error("map: first arg must be list");
    if (args[1]->type != Value::Type::Function) return Value::make_error("map: second arg must be function");
    std::string fn_name = std::get<FunctionValue>(args[1]->data).name;
    auto& lst = std::get<List>(args[0]->data);
    List result;
    for (auto& v : lst) {
        auto node = std::make_shared<ASTNode>();
        node->type = NodeType::Call;
        node->name = fn_name;
        node->args.push_back(std::make_shared<ASTNode>()); // 占位：应把 v 转为 AST literal
        node->args[0]->type = NodeType::Literal;
        if (v->type == Value::Type::String) {
            node->args[0]->value = std::get<std::string>(v->data);
        } else {
            node->args[0]->value = v->to_string();
        }
...
// filter(list, pred_fn) -> List：管道友好
static ValuePtr fn_filter(const std::vector<ValuePtr>& args) {
    if (args.size() < 2) return Value::make_error("filter: requires (list, pred)");
    if (args[0]->type != Value::Type::List) return Value::make_error("filter: first arg must be list");
    auto& lst = std::get<List>(args[0]->data);
    List result;
...
void register_stdlib(Evaluator& ev) {
    ...
    // map 需访问 Evaluator，用 lambda 捕获
    ev.register_builtin("map", [&ev](const std::vector<ValuePtr>& a){ return fn_map(a, ev); });
