...
class CommandParser {
public:
    CommandParser(std::shared_ptr<LlamaEngine> engine);
...
private:
    std::shared_ptr<LlamaEngine> ai_engine_;
...
public:
    SystemMonitorAI(std::shared_ptr<LlamaEngine> engine);
...
public:
    ConversationManager(std::shared_ptr<LlamaEngine> engine);
...
public:
    ...
    // 获取各个组件
    std::shared_ptr<LlamaEngine> getEngine();
